...
class SequentialDataset(Dataset):
    ...
    def data_augmentation(self):
        ...
        # new
        # num_records_to_save = 3
        # record_buffer = []
        # for i, uid in enumerate(self.inter_feat[self.uid_field].numpy()):
        #     if last_uid != uid:
        #         if last_uid is not None:
        #             uid_list.append(last_uid)
        #             for record in record_buffer[-num_records_to_save:]:
        #                 uid_list.append(last_uid)
        #                 item_list_index.append(record['item_index'])
        #                 target_index.append(record['target_index'])
        #                 item_list_length.append(record['item_length'])
        #         record_buffer = []
        #         seq_start = i
        #     if i - seq_start > max_item_list_len:
        #         seq_start = i - max_item_list_len
        #     record_buffer.append({'item_index': slice(seq_start, i), 'target_index': i, 'item_length': i - seq_start})
        #     last_uid = uid
        
        # if len(record_buffer) >= num_records_to_save:
        #     for record in record_buffer[-num_records_to_save:]:
        #         uid_list.append(last_uid)
        #         item_list_index.append(record['item_index'])
        #         target_index.append(record['target_index'])
