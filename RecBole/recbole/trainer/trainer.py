...
class MetaModel(nn.Module):
    def __init__(self, embedding_size, device):
        ...
        torch.nn.init.kaiming_normal_(self.meta_model[0].weight)
        torch.nn.init.kaiming_normal_(self.meta_model[2].weight)
        self.tau = nn.Parameter(torch.ones(1, device=self.device) * 10)
        self.device = device

    def forward(self, seq_embedding):
        logits = self.meta_model(seq_embedding)
        logits = logits.squeeze()
        # gumbel_noise = Gumbel(0, 1).sample(logits.shape).to(self.device)
        # tau = torch.clip(self.tau, min=1.0)
        # weights = torch.softmax((logits + gumbel_noise) / tau, dim=0)
        weights = F.gumbel_softmax(logits, tau=torch.clip(self.tau, min=1.0), dim=-1, hard=False)[..., 0]
...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        save_model_dir = "{}_{}".format(self.config['model'], self.config['dataset'])
        self.epoch_saved_model_dir = os.path.join(self.checkpoint_dir, 'analyze_tois', 'model', save_model_dir)
        if not os.path.exists(self.epoch_saved_model_dir):
            os.makedirs(self.epoch_saved_model_dir)
        ...
        self.meta_model = MetaModel(64, config['device']).to(config['device'])
        self.optimizer_meta = self._build_optimizer(params=self.meta_model.parameters())
        self.optimizer_rec = self._build_optimizer(params=self.model.parameters())
        self.meta_optimizer = MetaOptimizer(self.optimizer_meta, hpo_lr=0.001)
    ...
    def _train_epoch_bilevel(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        r"""Train the model in an epoch
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            scaler.scale(losses + sync_loss).backward()
            if self.clip_grad_norm:
                clip_grad_norm_(self.model.parameters(), **self.clip_grad_norm)
            scaler.step(self.optimizer)
            scaler.update()
            ...
            if epoch_idx > -1 and batch_idx % 30 == 0:
                # 使用DR4SR的meta optimizer优化
                losses, seq_embedding = self.model.calculate_loss_bilevel(interaction)
                weights = self.meta_model(seq_embedding)
                loss_weight = (weights * losses).mean()
                loss_mean = losses.mean()
                self.meta_optimizer.step(
                        val_loss=loss_mean,
                        train_loss=loss_weight,
                        aux_params = list(self.meta_model.parameters()),
            ...
            # elif batch_idx % 30 == 0:
                ...
                # self.optimizer_rec.step()  # 临时更新rec模型
                
                # # 步骤3：计算外层梯度（基于临时更新后的模型）
                # self.optimizer_meta.zero_grad()
                # # 重用之前的计算结果，避免重复forward
                # grad_meta = torch.autograd.grad(
                #     outputs=loss_weight,
                #     inputs=self.meta_model.parameters(),
                #     retain_graph=False  # 不需要保留计算图
                # )
                
                # # 步骤4：更新meta模型
                # for p, g in zip(self.meta_model.parameters(), grad_meta):
                #     p.grad = g.detach()  # 避免梯度积累
                # self.optimizer_meta.step()
                
                # # 步骤5：恢复初始参数并重新执行内层更新（永久更新）
                # for p_orig, p_current in zip(original_params, self.model.parameters()):
                #     p_current.data.copy_(p_orig.data)
                
                # # 再次执行内层更新（使用更新后的meta模型）
                # self.optimizer_rec.zero_grad()
                # losses, seq_embedding = self.model.calculate_loss_bilevel(interaction)
                # weights = self.meta_model(seq_embedding)
                # loss_weight = (weights * losses).mean()
                # loss_weight.backward()
                # self.optimizer_rec.step()  # 永久更新rec模型
                
                # # 记录损失
                # loss_mean = losses.mean()
                # total_loss = loss_mean.item() if total_loss is None else total_loss + loss_mean.item()

        return total_loss

    def _train_epoch_maxloss(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        r"""Train the model in an epoch
        ...
        """
        self.model.train()
        loss_func = loss_func or self.model.calculate_loss_maxloss
        total_loss = None
        iter_data = (
            tqdm(
                train_data,
    ...
    def _train_epoch_RM(self, train_data, model_RM, epoch_idx, loss_func=None, show_progress=False):
        r"""Train the model in an epoch
        ...
        """
        self.model.train()
        loss_func = loss_func or self.model.calculate_loss_RM
        total_loss = None
        iter_data = (
            tqdm(
                train_data,
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            with torch.autocast(device_type=self.device.type, enabled=self.enable_amp):
                losses = loss_func(interaction, model_RM)
    ...
    def _train_epoch_epoch(self, train_data, model_RM, model_epoch, epoch_idx, loss_func=None, show_progress=False):
        r"""Train the model in an epoch
        ...
        """
        self.model.train()
        loss_func = loss_func or self.model.calculate_loss_epoch
        total_loss = None
        iter_data = (
            tqdm(
                train_data,
        ...
        model_pth = os.path.join(self.checkpoint_dir, 'analyze_tois', 'model', f'{model_name}_{dataset_name}', f'{model_name}_{dataset_name}_{epoch_idx}.pth')
        model_epoch.load_state_dict(torch.load(model_pth, weights_only=False)['state_dict'])
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            with torch.autocast(device_type=self.device.type, enabled=self.enable_amp):
                losses, info = loss_func(interaction, model_RM)
                loss_epoch = self.model.calculate_RM(interaction, model_epoch)
                info_list.append([info, loss_epoch])
        ...
        # epoch训练完成后，计算DUR模型的loss，用于和epoch模型的loss进行对比，确保模型样本数相同
        with torch.no_grad():
            for batch_idx, interaction in enumerate(iter_data):
                interaction = interaction.to(self.device)
                loss = self.model.calculate_RM(interaction, self.model)
                info_list[batch_idx].append(loss)
    ...
    def fit(
        ...
        epoch_saved_model_file = os.path.join(self.epoch_saved_model_dir, f'{self.model_name}_{self.dataset_name}_-1.pth')
        torch.save(state, epoch_saved_model_file, pickle_protocol=4)
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            epoch_saved_model_file = os.path.join(self.epoch_saved_model_dir, f'{self.model_name}_{self.dataset_name}_{epoch_idx}.pth')
            torch.save(state, epoch_saved_model_file, pickle_protocol=4)
        ...
        # 存储loss_list
        # loss_save_pth = os.path.join(self.checkpoint_dir, 'analyze', 'loss_SASRec_Sports.pkl')
        # import pickle
        # with open(loss_save_pth, 'wb') as f:
    ...
    def fit_bilevel(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            # train
            training_start_time = time()
            train_loss = self._train_epoch_bilevel(
                train_data, epoch_idx, show_progress=show_progress
            )
            # loss_list.append(info_list)
            self.train_loss_dict[epoch_idx] = (
    ...
    def fit_maxloss(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            # train
            training_start_time = time()
            train_loss = self._train_epoch_maxloss(
                train_data, epoch_idx, show_progress=show_progress
            )
            # loss_list.append(info_list)
            self.train_loss_dict[epoch_idx] = (
    ...
    def cal_loss(self, train_data, model, loss_func=None, show_progress=False):
        model.eval()
        loss_func = model.calculate_RM
        total_loss = None
        iter_data = (
            tqdm(
                train_data,
        ...
        with torch.no_grad():
            for batch_idx, interaction in enumerate(iter_data):
                interaction = interaction.to(self.device)
                losses = loss_func(interaction, model)
    ...
    def fit_RM(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            # train
            training_start_time = time()
            train_loss = self._train_epoch_RM(
                train_data, model_RM, epoch_idx, show_progress=show_progress
            )
            self.train_loss_dict[epoch_idx] = (
                sum(train_loss) if isinstance(train_loss, tuple) else train_loss
            )
            ...
            model_name = self.config['model']
            dataset_name = self.config['dataset']
            epoch_saved_model_file = os.path.join(self.epoch_saved_model_dir, f'{model_name}DUR_{dataset_name}_{epoch_idx}.pth')
            torch.save(state, epoch_saved_model_file, pickle_protocol=4)
    ...
    def fit_epoch(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            # train
            training_start_time = time()
            train_loss, loss_analyze = self._train_epoch_epoch(
                train_data, model_RM, model_epoch, epoch_idx, show_progress=show_progress
            )
            loss_analyze_list.append(loss_analyze)
            self.train_loss_dict[epoch_idx] = (
                sum(train_loss) if isinstance(train_loss, tuple) else train_loss
            )
            training_end_time = time()
        ...
        return self.best_valid_score, self.best_valid_result, loss_analyze_list
...
class DecisionTreeTrainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        saved_model_file = "{}-{}.pth".format(self.config["model"], get_local_time())
        self.saved_model_file = os.path.join(self.checkpoint_dir, 'analyze', saved_model_file)
