...
class BERT4Rec(SequentialRecommender):
    ...
    def calculate_RM(self, interaction, model_RM):
        with torch.no_grad():
            # 计算RM的loss值作为数据选择的基线
            masked_item_seq = interaction[model_RM.MASK_ITEM_SEQ]
            pos_items = interaction[model_RM.POS_ITEMS]
            neg_items = interaction[model_RM.NEG_ITEMS]
            masked_index = interaction[model_RM.MASK_INDEX]

            seq_output = model_RM.forward(masked_item_seq)
            pred_index_map = model_RM.multi_hot_embed(
                masked_index, masked_item_seq.size(-1)
            )  # [B*mask_len max_len]
            # [B mask_len] -> [B mask_len max_len] multi hot
            pred_index_map = pred_index_map.view(
            ...
            seq_output = torch.bmm(pred_index_map, seq_output)  # [B mask_len H]
            loss_fct = nn.CrossEntropyLoss(reduction="none")
            test_item_emb = model_RM.item_embedding.weight[: model_RM.n_items]  # [item_num H]
            logits = (
                torch.matmul(seq_output, test_item_emb.transpose(0, 1))
                + model_RM.output_bias
            )  # [B mask_len item_num]
            targets = (masked_index > 0).float().view(-1)  # [B*mask_len]

            loss = loss_fct(logits.view(-1, test_item_emb.size(0)), pos_items.view(-1)) * targets
            return loss

    def data_selection(self, loss, loss_RM):
        delta = loss - loss_RM
        # 选择delta值前百分之K的数据，生成mask张量，对loss做mask
        # 挑选出loss中不为0的位置，然后从对应的delta中选择其中的前K%的数据
        non_zero = (loss != 0)
        loss = loss[non_zero]
        delta = delta[non_zero]
        topk = int(loss.shape[0] * 0.9)
