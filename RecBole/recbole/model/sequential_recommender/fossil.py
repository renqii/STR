...
class FOSSIL(SequentialRecommender):
    ...
    def calculate_RM(self, interaction, model_RM):
        with torch.no_grad():
            seq_item = interaction[model_RM.ITEM_SEQ]
            user = interaction[model_RM.USER_ID]
            seq_item_len = interaction[model_RM.ITEM_SEQ_LEN]
            seq_output = model_RM.forward(seq_item, seq_item_len, user)
            pos_items = interaction[model_RM.POS_ITEM_ID]

            test_item_emb = model_RM.item_embedding.weight
    ...
    def calculate_loss_RM(self, interaction, model_RM):
        ...
        if self.loss_type == "BPR":
            ...
            logits = torch.matmul(seq_output, test_item_emb.transpose(0, 1))
            loss_ori = self.loss_fct(logits, pos_items)
