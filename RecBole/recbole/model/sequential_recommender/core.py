...
class CORE(SequentialRecommender):
    ...
    def calculate_RM(self, interaction, model_RM):
        with torch.no_grad():
            ...
            seq_output = model_RM.forward(item_seq)
            pos_items = interaction[model_RM.POS_ITEM_ID]

            all_item_emb = model_RM.item_embedding.weight
            all_item_emb = model_RM.item_dropout(all_item_emb)
            # Robust Distance Measuring (RDM)
            all_item_emb = F.normalize(all_item_emb, dim=-1)
            logits = (
                torch.matmul(seq_output, all_item_emb.transpose(0, 1)) / model_RM.temperature
            )
