...
class RepeatNet(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.loss_fct = nn.NLLLoss(reduce=False, ignore_index=0)
    ...
    def calculate_RM(self, interaction, model_RM):
        with torch.no_grad():
            ...
            pos_item = interaction[model_RM.POS_ITEM_ID]
            prediction = model_RM.forward(item_seq, item_seq_len)
            loss = self.loss_fct((prediction + 1e-8).log(), pos_item)
            return loss
    ...
    def calculate_loss_RM(self, interaction, model_RM):
        ...
        loss_ori = self.loss_fct((prediction + 1e-8).log(), pos_item)
        loss_RM = self.calculate_RM(interaction, model_RM)
        loss_chosen = self.data_selection(loss_ori, loss_RM)
        loss = loss_chosen.mean()
